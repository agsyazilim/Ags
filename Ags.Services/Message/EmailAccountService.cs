    ...
    public partial class EmailAccountService : IEmailAccountService
    {
        ...
        public virtual void DeleteEmailAccount(EmailAccount emailAccount)
        {
            ...
            if (GetAllEmailAccounts().Count == 1)
                throw new AgsException("You cannot delete this email account. At least one account is required.");
        ...
        {
            IOrderedQueryable<EmailAccount> query = from ea in _emailAccountRepository.Table
                        orderby ea.Id
                        select ea;
            List<EmailAccount> emailAccounts = query.ToList();
