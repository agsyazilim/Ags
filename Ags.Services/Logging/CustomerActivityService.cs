...
namespace Ags.Services.Logging
{
    ...
    public class CustomerActivityService : ICustomerActivityService
    {
        ...
        public CustomerActivityService(IDbContext dbContext,
            ...
            IWebHelper webHelper, IHttpContextAccessor httpContextAccessor, UserManager<ApplicationUser> userManager, ICustomerService customerService)
        {
        ...
        {
            //cache
            return _cacheManager.Get(AgsLoggingDefaults.ActivityTypeAllCacheKey, () =>
            {
                List<ActivityLogTypeForCaching> result = new List<ActivityLogTypeForCaching>();
                IList<ActivityLogType> activityLogTypes = GetAllActivityTypes();
                foreach (ActivityLogType alt in activityLogTypes)
                {
                    ActivityLogTypeForCaching altForCaching = new ActivityLogTypeForCaching
                    {
                        Id = alt.Id,
        ...
        public virtual void InsertActivityType(ActivityLogType activityLogType)
        {
            ...
            _cacheManager.RemoveByPattern(AgsLoggingDefaults.ActivityTypePatternCacheKey);
        }
        ...
        {
            IOrderedQueryable<ActivityLogType> query = from alt in _activityLogTypeRepository.Table
                        orderby alt.Name
                        select alt;
            List<ActivityLogType> activityLogTypes = query.ToList();
        ...
        {
            IQueryable<ActivityLog> query = _activityLogRepository.Table;
