    ...
    public partial class UrlRecordService : IUrlRecordService
    {
        ...
        public UrlRecordService(
            IRepository<UrlRecord> urlRecordRepository,
             ISettingService settingService, IStaticCacheManager cacheManager)
        {
        ...
        protected virtual string ToUnichar(string hexString)
        {
            ...
            string returnChar = Encoding.Unicode.GetString(b);
            return returnChar;
        }
        ...
        public virtual void DeleteUrlRecord(UrlRecord urlRecord)
        {
            ...
            //cache
            _cacheManager.RemoveByPattern(AgsSeoDefaults.UrlRecordPatternCacheKey);
        }
        ...
        {
            IQueryable<UrlRecord> query = _urlRecordRepository.Table;
        ...
        public virtual UrlRecord GetBySlug(string slug)
        {
            ...
            IOrderedQueryable<UrlRecord> query = from ur in _urlRecordRepository.Table
                        where ur.Slug == slug
                        //first, try to find an active record
                        orderby ur.IsActive descending, ur.Id
                        select ur;
            UrlRecord urlRecord = query.FirstOrDefault();
            return urlRecord;
        }
        ...
        public virtual IPagedList<UrlRecord> GetAllUrlRecords(string slug = "", int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            PagedList<UrlRecord> urlRecords = new PagedList<UrlRecord>(query, pageIndex, pageSize);
            return urlRecords;
        }
        ...
        {
            IQueryable<UrlRecord> source = _urlRecordRepository.Table;
            IQueryable<string> query = from ur in source
            ...
            string slug = query.FirstOrDefault() ?? string.Empty;
        ...
        public virtual void SaveSlug<T>(T entity, string slug) where T : BaseEntity, ISlugSupported
        {
            ...
            IOrderedQueryable<UrlRecord> query = from ur in _urlRecordRepository.Table
                        ...
                        orderby ur.Id descending
                        select ur;
            List<UrlRecord> allUrlRecords = query.ToList();
            UrlRecord activeUrlRecord = allUrlRecords.FirstOrDefault(x => x.IsActive);
        ...
        public virtual string GetSeName<T>(T entity, bool returnDefaultValue = true) where T : BaseEntity, ISlugSupported
        {
            if (entity == null)
                throw new ArgumentNullException(nameof(entity));
        ...
        /// <param name="returnDefaultValue">A value indicating whether to return default value (if language specified one is not found)</param>
        /// <returns>Search engine  name (slug)</returns>
        public virtual string GetSeName(int entityId, string entityName,
            bool returnDefaultValue = true)
        {
            ...
            if (string.IsNullOrEmpty(result) && returnDefaultValue)
                result = GetActiveSlug(entityId, entityName);
        ...
        public virtual string ValidateSeName(int entityId, string entityName, string seName, string name, bool ensureNotEmpty)
        {
            ...
            if (string.IsNullOrWhiteSpace(seName) && !string.IsNullOrWhiteSpace(name))
                seName = name;
            SeoSettings seosettings = _settingService.LoadSetting<SeoSettings>();
            //validation
            seName = GetSeName(seName, seosettings.ConvertNonWesternChars, seosettings.AllowUnicodeCharsInUrls);

            //max length
            seName = CommonHelper.EnsureMaximumLength(seName, AgsSeoDefaults.SearchEngineNameLength);
            ...
            int i = 2;
            string tempSeName = seName;
            while (true)
            {
                //check whether such slug already exists (and that is not the current entity)
                UrlRecord urlRecord = GetBySlug(tempSeName);
                bool reserved1 = urlRecord != null && !(urlRecord.EntityId == entityId && urlRecord.EntityName.Equals(entityName, StringComparison.InvariantCultureIgnoreCase));
                //and it's not in the list of reserved slugs
                bool reserved2 = seosettings.ReservedUrlRecordSlugs.Contains(tempSeName, StringComparer.InvariantCultureIgnoreCase);
        ...
        public UrlRecordForCaching GetBySlugCached(string slug)
        {
            ...
            var key = string.Format(AgsSeoDefaults.UrlRecordBySlugCacheKey, slug);
