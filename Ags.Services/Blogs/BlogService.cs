...
namespace Ags.Services.Blogs
{
    ...
    public partial class BlogService : IBlogService
    {
        ...
        {
            IQueryable<BlogPost> query = _blogPostRepository.Table;
        ...
        /// <returns>Blog posts</returns>
        public virtual IPagedList<BlogPost> GetAllBlogPosts(int customerId = 0,
            DateTime? dateFrom = null, DateTime? dateTo = null,int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            IQueryable<BlogPost> query = _blogPostRepository.Table;
            if (customerId > 0)
            {
            ...
            PagedList<BlogPost> blogPosts = new PagedList<BlogPost>(query, pageIndex, pageSize);
            return blogPosts;
        }
        ...
        /// <param name="showHidden">A value indicating whether to show hidden records</param>
        /// <returns>Blog posts</returns>
        public virtual IPagedList<BlogPost> GetAllBlogPostsByTag(int customerId = 0, string tag = "",
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            tag = tag.Trim();

            //we load all records and only then filter them by tag
            IPagedList<BlogPost> blogPostsAll = GetAllBlogPosts(customerId: customerId,showHidden: showHidden);
            List<BlogPost> taggedBlogPosts = new List<BlogPost>();
            ...
            //server-side paging
            PagedList<BlogPost> result = new PagedList<BlogPost>(taggedBlogPosts, pageIndex, pageSize);
            return result;
        }
        ...
        public virtual IList<string> ParseTags(BlogPost blogPost)
        {
            ...
            List<string> tags = blogPost.Tags.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries)
                .Select(tag => tag?.Trim())
        ...
        public List<int> GetBlogListIds()
        {
            var query = from post in _blogPostRepository.Table select post.Id;
        ...
        {
            IQueryable<BlogComment> query = _blogCommentRepository.Table;
        ...
        public virtual IList<BlogComment> GetBlogCommentsByIds(int[] commentIds)
        {
            ...
            IQueryable<BlogComment> query = from bc in _blogCommentRepository.Table
                        where commentIds.Contains(bc.Id)
                        select bc;
            List<BlogComment> comments = query.ToList();
            //sort by passed identifiers
            List<BlogComment> sortedComments = new List<BlogComment>();
            foreach (int id in commentIds)
            {
                BlogComment comment = comments.Find(x => x.Id == id);
                if (comment != null)
                {
        ...
        {
            IQueryable<BlogComment> query = _blogCommentRepository.Table.Where(comment => comment.BlogPostId == blogPost.Id);
