                        ...
                        case FormValueRequirement.StartsWith:
                            {
                                ...
                                {
                                    //validate "value"
                                    foreach (string formValue in routeContext.HttpContext.Request.Form.Keys)
                                        if (formValue.StartsWith(buttonName, StringComparison.InvariantCultureIgnoreCase))
                                        {
                                            Microsoft.Extensions.Primitives.StringValues value = routeContext.HttpContext.Request.Form[formValue];
                                            if (!string.IsNullOrEmpty(value))
                                                return true;
                                        }
                                }
