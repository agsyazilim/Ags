...
namespace Ags.Web.Framework.Mvc.Rss
{
    ...
    public class RssItem
    {
        ...
        public RssItem(XContainer item)
        {
            ...
            string content = item.Element("content")?.Value ?? string.Empty;
            if (string.IsNullOrEmpty(content))
                content = item.Element("description")?.Value ?? string.Empty;
            Uri link = new Uri(item.Element("link")?.Value ?? string.Empty);
            string pubDateValue = item.Element("pubDate")?.Value;
            DateTimeOffset pubDate = pubDateValue == null ? DateTimeOffset.Now : DateTimeOffset.ParseExact(pubDateValue, "r", null);
            string id = item.Element("guid")?.Value ?? string.Empty;
        ...
        {
            XElement element = new XElement("item", Id, Link, Title, Content);
