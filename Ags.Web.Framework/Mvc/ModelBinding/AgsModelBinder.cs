...
{
    /// <summary>
    /// Represents model binder for the binding models inherited from the BaseAgsModel
    /// </summary>
    public class AgsModelBinder : ComplexTypeModelBinder
    {
        #region Ctor
        ...
        public AgsModelBinder(IDictionary<ModelMetadata, IModelBinder> propertyBinders, ILoggerFactory loggerFactory)
        ...
        protected override object CreateModel(ModelBindingContext bindingContext)
        {
            ...
            if (model is BaseAgsModel)
                (model as BaseAgsModel).BindModel(bindingContext);
            ...
            ModelMetadata propertyMetadata, ModelBindingResult bindingResult)
        {
            ...
            if (bindingContext.Model is BaseAgsModel && !string.IsNullOrEmpty(valueAsString))
            {
                //excluding properties with [NoTrim] attribute
                bool? noTrim = (propertyMetadata as DefaultModelMetadata)?.Attributes?.Attributes?.OfType<NoTrimAttribute>().Any();
