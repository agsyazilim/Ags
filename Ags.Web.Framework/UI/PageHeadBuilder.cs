...
namespace Ags.Web.Framework.UI
{
    /// <summary>
    /// Page head builder
    /// </summary>
    public partial class PageHeadBuilder : IPageHeadBuilder
    {
        ...
        {
            string result = "";
            string specificTitle = string.Join(_seoSettings.PageTitleSeparator, _titleParts.AsEnumerable().Reverse().ToArray());
        ...
        public virtual string GenerateMetaDescription()
        {
            ...
            string result = !string.IsNullOrEmpty(metaDescription) ? metaDescription : _seoSettings.DefaultMetaDescription;
            return result;
        }
        ...
        public virtual string GenerateMetaKeywords()
        {
            ...
            string result = !string.IsNullOrEmpty(metaKeyword) ? metaKeyword : _seoSettings.DefaultMetaKeywords;
            return result;
        }
        ...
        public virtual string GenerateScripts(IUrlHelper urlHelper, ResourceLocation location, bool? bundleFiles = null)
        {
            ...
            bool debugModel = _hostingEnvironment.IsDevelopment();
            ...
            {
                ScriptReferenceMeta[] partsToBundle = _scriptParts[location]
                    .Where(x => !x.ExcludeFromBundle)
                    .Distinct()
                    .ToArray();
                ScriptReferenceMeta[] partsToDontBundle = _scriptParts[location]
                    .Where(x => x.ExcludeFromBundle)
                    .Distinct()
                    .ToArray();
                ...
                if (partsToBundle.Any())
                {
                    ...
                    foreach (ScriptReferenceMeta item in partsToBundle)
                    {
                        ...
                        string src = path.Value.TrimStart('/');
                    ...
                    //output file
                    string outputFileName = GetBundleFileName(partsToBundle.Select(x => debugModel ? x.DebugSrc : x.Src).ToArray());
                    ...
                    lock (s_lock)
                    {
                        ...
                        string cacheKey = $"ags.minification.shouldrebuild.js-{outputFileName}";
                        bool shouldRebuild = _cacheManager.Get(cacheKey, () => true, RecheckBundledFilesPeriod);
                        if (shouldRebuild)
                        {
                ...
                //parts to not bundle
                foreach (ScriptReferenceMeta item in partsToDontBundle)
                {
                    string src = debugModel ? item.DebugSrc : item.Src;
            ...
            else
            {
                ...
                StringBuilder result = new StringBuilder();
                foreach (ScriptReferenceMeta item in _scriptParts[location].Distinct())
                {
                    string src = debugModel ? item.DebugSrc : item.Src;
                    result.AppendFormat("<script {1}src=\"{0}\"></script>", urlHelper.Content(src), item.IsAsync ? "async " : "");
                    result.Append(Environment.NewLine);
        ...
        public virtual string GenerateCssFiles(IUrlHelper urlHelper, ResourceLocation location, bool? bundleFiles = null)
        {
            ...
            if (bundleFiles.Value)
            {
                CssReferenceMeta[] partsToBundle = _cssParts[location]
                    .Where(x => !x.ExcludeFromBundle)
                    .Distinct()
                    .ToArray();
                CssReferenceMeta[] partsToDontBundle = _cssParts[location]
                    .Where(x => x.ExcludeFromBundle)
                    .Distinct()
                    .ToArray();
                ...
                if (partsToBundle.Any())
                {
                    ...
                    Bundle bundle = new Bundle();
                    foreach (CssReferenceMeta item in partsToBundle)
                    {
                        string src = debugModel ? item.DebugSrc : item.Src;
                        src = urlHelper.Content(src);
                        //check whether this file exists
                        string srcPath = _fileProvider.Combine(_hostingEnvironment.ContentRootPath, src.Remove(0, 1).Replace("/", "\\"));
                    ...
                    //output file
                    string outputFileName = GetBundleFileName(partsToBundle.Select(x => { return debugModel ? x.DebugSrc : x.Src; }).ToArray());
                    ...
                    lock (s_lock)
                    {
                        ...
                        //we periodically re-check already bundles file
                        //so if we have minification enabled, it could take up to several minutes to see changes in updated resource files (or just reset the cache or restart the site)
                        string cacheKey = $"Ags.minification.shouldrebuild.css-{outputFileName}";
                        bool shouldRebuild = _cacheManager.Get(cacheKey, () => true, RecheckBundledFilesPeriod);
                        if (shouldRebuild)
                        {
                            //store json file to see a generated config file (for debugging purposes)
                ...
                //parts not to bundle
                foreach (CssReferenceMeta item in partsToDontBundle)
                {
                    string src = debugModel ? item.DebugSrc : item.Src;
            ...
            else
            {
                //bundling is disabled
                StringBuilder result = new StringBuilder();
                foreach (CssReferenceMeta item in _cssParts[location].Distinct())
                {
                    string src = debugModel ? item.DebugSrc : item.Src;
                    result.AppendFormat("<link href=\"{0}\" rel=\"stylesheet\" type=\"{1}\" />", urlHelper.Content(src), MimeTypes.TextCss);
                    result.AppendLine();
        ...
        public virtual string GenerateHeadCustom()
        {
            ...
            List<string> distinctParts = _headCustomParts.Distinct().ToList();
