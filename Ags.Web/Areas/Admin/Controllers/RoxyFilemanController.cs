...
namespace Ags.Web.Areas.Admin.Controllers
{
    ...
    public class RoxyFilemanController : BaseAdminController
    {
        ...
        {
            string filePath = GetFullPath(CONFIGURATION_FILE);

            //create file if not exists
            _fileProvider.CreateFile(filePath);

            //try to read existing configuration
            string existingText = _fileProvider.ReadAllText(filePath, Encoding.UTF8);
            ...
            string currentPathBase = this.HttpContext.Request.PathBase.ToString();
            ...
            var configuration = new
            {
                ...
                LANG = existingConfiguration?.LANG ?? "tr",
            ...
            //save the file
            string text = JsonConvert.SerializeObject(configuration, Formatting.Indented);
            ...
            try
            {
                ...
                switch (action.ToUpper())
                {
                    ...
                    case "RENAMEFILE":
                        ...
                        break;
                    case "GENERATETHUMB":
                        int.TryParse(HttpContext.Request.Query["width"].ToString().Replace("px", string.Empty), out int w);
                        int.TryParse(HttpContext.Request.Query["height"].ToString().Replace("px", string.Empty), out int h);
        ...
        {
            string filesRoot = GetSetting("FILES_ROOT");

            string sessionPathKey = GetSetting("SESSION_PATH_KEY");
        ...
        {
            string languageCode = GetSetting("LANG");
            string languageFile = $"{LANGUAGE_DIRECTORY}/{languageCode}.json";

            if (!_fileProvider.FileExists(GetFullPath(languageFile)))
                languageFile = $"{LANGUAGE_DIRECTORY}/tr.json";
        ...
        protected virtual bool CanHandleFile(string path)
        {
            ...
            string fileExtension = _fileProvider.GetFileExtension(path).Replace(".", string.Empty).ToLower();

            string forbiddenUploads = GetSetting("FORBIDDEN_UPLOADS").Trim().ToLower();
            if (!string.IsNullOrEmpty(forbiddenUploads))
            {
                ArrayList forbiddenFileExtensions = new ArrayList(Regex.Split(forbiddenUploads, "\\s+"));
                result = !forbiddenFileExtensions.Contains(fileExtension);
            }

            string allowedUploads = GetSetting("ALLOWED_UPLOADS").Trim().ToLower();
            if (string.IsNullOrEmpty(allowedUploads))
                return result;

            ArrayList allowedFileExtensions = new ArrayList(Regex.Split(allowedUploads, "\\s+"));
        ...
        protected virtual ArrayList GetDirectories(string parentDirectoryPath)
        {
            ...
            string[] directoryNames = _fileProvider.GetDirectories(parentDirectoryPath);
        ...
        protected virtual List<string> GetFiles(string directoryPath, string type)
        {
            ...
            foreach (string fileName in _fileProvider.GetFiles(directoryPath))
            {
        ...
        {
            string[] existingFiles = _fileProvider.GetFiles(sourcePath);
            string[] existingDirectories = _fileProvider.GetDirectories(sourcePath);
            ...
            {
                string filePath = _fileProvider.Combine(destinationPath, _fileProvider.GetFileName(file));
            ...
            {
                string directoryPath = _fileProvider.Combine(destinationPath, _fileProvider.GetDirectoryName(directory));
        ...
        {
            string filePath = GetFullPath(GetVirtualPath(sourcePath));

            if (!_fileProvider.FileExists(filePath))
                throw new Exception(GetLanguageResource("E_CopyFileInvalisPath"));

            destinationPath = GetFullPath(GetVirtualPath(destinationPath));
            string newFileName = GetUniqueFileName(destinationPath, _fileProvider.GetFileName(filePath));
            try
            {
        ...
        protected virtual async Task CreateDirectoryAsync(string parentDirectoryPath, string name)
        {
            ...
            try
            {
                string path = _fileProvider.Combine(parentDirectoryPath, name);
        ...
        public async Task DownloadDirectoryAsync(string path)
        {
            ...
            string zipName = _fileProvider.GetFileName(fullPath) + ".zip";
        ...
        {
            string filePath = GetFullPath(GetVirtualPath(path));
            if (_fileProvider.FileExists(filePath))
            {
                HttpContext.Response.Clear();
        ...
        protected virtual async Task RenameFileAsync(string sourcePath, string newName)
        {
            ...
            try
            {
                string destinationPath = _fileProvider.Combine(_fileProvider.GetDirectoryName(fullSourcePath), newName);
                _fileProvider.FileMove(fullSourcePath, destinationPath);
        ...
        protected virtual void CreateThumbnail(string path, int width, int height)
        {
            ...
            {
                using (Bitmap image = new Bitmap(Image.FromStream(stream)))
                {
                    int cropX = 0;
                    int cropY = 0;

                    double imgRatio = image.Width / (double)image.Height;
                    ...
                    int cropWidth = Convert.ToInt32(Math.Floor(image.Height * cropRatio));
                    int cropHeight = Convert.ToInt32(Math.Floor(cropWidth / cropRatio));
                    ...
                    using (Bitmap cropImg = image.Clone(new Rectangle(cropX, cropY, cropWidth, cropHeight), PixelFormat.DontCare))
                    {
        ...
        {
            string fileExtension = _fileProvider.GetFileExtension(path).ToLower();
        ...
        {
            string result = GetSuccessResponse();
            bool hasErrors = false;
            try
            {
                directoryPath = GetFullPath(GetVirtualPath(directoryPath));
                foreach (IFormFile formFile in HttpContext.Request.Form.Files)
                {
                    string fileName = formFile.FileName;
                    if (CanHandleFile(fileName))
                    {
                        string uniqueFileName = GetUniqueFileName(directoryPath, _fileProvider.GetFileName(fileName));
                        string destinationFile = _fileProvider.Combine(directoryPath, uniqueFileName);
