...
namespace Ags.Data.Core
{
    ...
    public partial class WebHelper : IWebHelper
    {
        ...
        public virtual string GetCurrentIpAddress()
        {
            ...
            try
            {
                ...
                if (_httpContextAccessor.HttpContext.Request.Headers != null)
                {
                    ...
                    StringValues forwardedHeader = _httpContextAccessor.HttpContext.Request.Headers[forwardedHttpHeaderKey];
        ...
        public virtual string GetThisPageUrl(bool includeQueryString, bool? useSsl = null, bool lowercaseUrl = false)
        {
            ...
            //get store location
            string storeLocation = GetStoreLocation(useSsl ?? IsCurrentConnectionSecured());

            //add local path to the URL
            string pageUrl = $"{storeLocation.TrimEnd('/')}{_httpContextAccessor.HttpContext.Request.Path}";
        ...
        public virtual string GetStoreHost(bool useSsl)
        {
            ...
            StringValues hostHeader = _httpContextAccessor.HttpContext.Request.Headers[HeaderNames.Host];
            if (StringValues.IsNullOrEmpty(hostHeader))
                return string.Empty;

            //add scheme to the URL
            string storeHost = $"{(useSsl ? Uri.UriSchemeHttps : Uri.UriSchemeHttp)}{Uri.SchemeDelimiter}{hostHeader.FirstOrDefault()}";
        ...
        public virtual string GetStoreLocation(bool? useSsl = null)
        {
            ...
            //get store host
            string storeHost = GetStoreHost(useSsl ?? IsCurrentConnectionSecured());
            ...
            if (string.IsNullOrEmpty(storeHost))
            {
                ...
                storeLocation = "http://localhost:5000"
        ...
        public virtual string RemoveQueryString(string url, string key, string value = null)
        {
            ...
            //get current query parameters
            Uri uri = new Uri(url);
            List<KeyValuePair<string, string>> queryParameters = QueryHelpers.ParseQuery(uri.Query)
        ...
        public virtual void RestartAppDomain(bool makeRedirect = false)
        {
            ...
            if (!success)
            {
                throw new AgsException("agshaber needs to be restarted due to a configuration change, but was unable to do so." + Environment.NewLine +
        ...
        public virtual bool IsPostBeingDone
        {
            get
            {
                if (_httpContextAccessor.HttpContext.Items[AgsHttpDefaults.IsPostBeingDoneRequestItem] == null)
                    return false;

                return Convert.ToBoolean(_httpContextAccessor.HttpContext.Items[AgsHttpDefaults.IsPostBeingDoneRequestItem]);
            }

            set => _httpContextAccessor.HttpContext.Items[AgsHttpDefaults.IsPostBeingDoneRequestItem] = value;
        }
