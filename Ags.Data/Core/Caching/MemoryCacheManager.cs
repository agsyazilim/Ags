...
// ***********************************************************************
// <copyright file="MemoryCacheManager.cs" company="Nop Solutions, Ltd">
...
namespace Ags.Data.Core.Caching
{
    ...
    /// Implements the <see cref="ILocker" />
    /// Implements the <see cref="ILocker" />
    /// </summary>
    ...
    public partial class MemoryCacheManager : ILocker, IStaticCacheManager
    {
        ...
        public virtual T Get<T>(string key, Func<T> acquire, int? cacheTime = null)
        {
            ...
            if ((cacheTime ?? AgsCachingDefaults.CacheTime) > 0)
                Set(key, result, cacheTime ?? AgsCachingDefaults.CacheTime);
        ...
        public virtual void RemoveByPattern(string pattern)
        {
            ...
            System.Collections.Generic.List<string> matchesKeys = _allKeys.Where(p => p.Value).Select(p => p.Key).Where(key => regex.IsMatch(key)).ToList();

            //remove matching values
            foreach (string key in matchesKeys)
            {
                _cache.Remove(RemoveKey(key));
            }
        }
